// MARK: - OperationStoreSubscriptions

package final class OperationSubscriptions<QueryHandler: Sendable>: Sendable {
...
extension OperationSubscriptions {
...
extension OperationSubscriptions {
  ...
  ) -> (subscription: OperationSubscription, isFirst: Bool) {
    self.state.withLock { state in
      let id = state.currentId
      defer { state.currentId += 1 }
      state.handlers[id] = (isTemporary, handler)
      let subscription = OperationSubscription { [weak self] in
