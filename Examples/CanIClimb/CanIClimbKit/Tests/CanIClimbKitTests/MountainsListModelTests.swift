  ...
  struct MountainsListModelTests {
    ...
    func refetchesWhenSearchCategoryChanges() async throws {
      ...
      try await withDependencies {
        ...
        expectNoDifference(model.mountains, [Page(id: 0, value: recommendedResult)])

        model.category = .planned
        _ = try await model.$mountains.initialPageActiveTasks.first?.runIfNeeded()
        expectNoDifference(model.mountains, [Page(id: 0, value: plannedResult)])
      }
    }
    ...
    func debouncesTextualSearches() async throws {
      ...
      try await withDependencies {
        ...
        _ = try await model.$mountains.initialPageActiveTasks.first?.runIfNeeded()
        expectNoDifference(model.mountains, [Page(id: 0, value: noTextResult)])
        ...
        _ = try await model.$mountains.initialPageActiveTasks.first?.runIfNeeded()
        expectNoDifference(model.mountains, [Page(id: 0, value: textResult)])
      }
    }
